...
export default class CosmereUnofficialActorBase extends CosmereUnofficialDataModel {
	...
	static migrateData(source) {
		...
		if (!source.health) {
			...
			if (source.health.value &&
				(!isNumber(source.health.value) || !isNumeric(source.health.value))
			) {
				source.health.value = 10;
			}
			if (source.health.max &&
				(!isNumber(source.health.max) || !isNumeric(source.health.max))) {
				source.health.max = 10;
			}
		}
		if (!source.focus) {
			source.focus = {
			...
			if (source.focus.value &&
				(!isNumber(source.focus.value) || !isNumeric(source.focus.value))
			) {
				source.focus.value = 2;
			}
			if (source.focus.max &&
				(!isNumber(source.focus.max) || !isNumeric(source.focus.max))
			) {
				source.focus.max = 2;
			}
		}
		if (!source.investiture) {
			...
			if (source.investiture.value &&
				(!isNumber(source.investiture.value) || !isNumeric(source.investiture.value))
			) {
				source.investiture.value = 0;
			}
			if (source.investiture.max &&
				(!isNumber(source.investiture.max) || !isNumeric(source.investiture.max))
			) {
